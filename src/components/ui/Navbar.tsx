  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        {/* Logo / Name */}
        <Link href="#home" className="font-mono text-teal text-sm font-semibold tracking-wider hover:opacity-80 transition-opacity">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            aria-label="Menu"
          >
            <span className={`block w-5 h-0.5 bg-mist transition-all ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} />
            <span className={`block w-5 h-0.5 bg-mist transition-all ${menuOpen ? 'opacity-0' : ''}`} />
            <span className={`block w-5 h-0.5 bg-mist transition-all ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
          </button>
        </div>
      </div>
